...
- `Python` 3.13
- `Uv` для управления python-пакетами
...
- Для БД-миграций – распространённый `alembic` с асинхронным шаблоном: 
...
- Работа с БД происходит в репозиториях. 
  Вход-выход в типах-примитивах (`dict`, `int`, …) Python. 
  Репозиториями пользуются только их сервисы. 
  Сервисы преобразуют из/в pydantic модели, с которыми и работает приложение.
...
Для работы контейнеров в Docker нужен файл с переменными окружения `docker/.env`.
...
- Swagger по API – http://localhost:8000/docs/
